#include "legged_controllers/LeggedPhasePred.h"
...
void LeggedPhasePred::initialize() {
    ...
    forceModels_[0] = {350.0, 170.0, 15.0, 50.0, 10.0, 50.0};  // LF
    forceModels_[1] = {350.0, 170.0, 15.0, 50.0, 10.0, 50.0};  // RF
    forceModels_[2] = {350.0, 170.0, 15.0, 50.0, 10.0, 50.0};  // LH
    forceModels_[3] = {350.0, 170.0, 15.0, 50.0, 10.0, 50.0};  // RH
...
double LeggedPhasePred::normpdf(double x, double mu, double sigma) {
...
double LeggedPhasePred::estimateFootContactForce(const Eigen::Vector3d& jointTorque, const Eigen::Matrix3d& Jt) {
    Eigen::Vector3d contactForce = Jt.colPivHouseholderQr().solve(jointTorque);
    return contactForce.norm();
}

void LeggedPhasePred::decidePhase(int leg, double contact_force) {
...
void LeggedPhasePred::update() {
    ...
    for (const auto& handle : hybridJointHandles_) {
    ...
    for (int leg = 0; leg < 4; ++leg) {
        ...
        // ROS_INFO_STREAM("Processing leg " << leg << ": " << footName);
        // ROS_INFO_STREAM("HAA: " << HAAJointNames << ", HFE: " << HFEJointNames << ", KFE: " << KFEJointNames);

        Eigen::MatrixXd J_full(6, model.nv);
        pinocchio::getFrameJacobian(model, data, frameId, pinocchio::ReferenceFrame::LOCAL, J_full);
        ...
        Eigen::Vector3d footForce_local = (J_leg.transpose().inverse()) * (tau_leg);

        // Normalize the force vector
        contactForces_[leg] = footForce_local.norm();
